...
export default function MarketplacePage({ visible, deployed, listings, account, cards, busy, tokenId, setTokenId, price, setPrice, filter, setFilter, sort, setSort, onList, onBuy, onCancel }) {
  ...
  return <section className={`features-section app-page ${visible ? "page-active" : ""}`} id="marketplace">
    ...
    <div className="market-toolbar"><div className="market-tabs"><button className="active">Items</button><a href="#collection">My collection</a></div><div className="market-result-count"><strong>{visibleListings.length}</strong> items</div><select aria-label="Sort marketplace" value={sort} onChange={(e) => setSort(e.target.value)}><option value="low">Price: low to high</option><option value="high">Price: high to low</option></select></div>
    <div className="market-layout">
      <aside className="market-sidebar">
        <div className="market-filter-block"><strong>Status</strong><button className="status-filter"><i /> Buy now</button></div>
        ...
        <details className="market-list-panel"><summary><span>＋</span> List an item</summary><div className="list-panel-body"><small>SELL FROM YOUR WALLET</small><h3>Create listing</h3><label>Choose from your collection</label><div className="creature-picker">{rarities.map((rarity, id) => <button type="button" key={rarity.name} className={`${tokenId === id ? "active" : ""} ${rarity.className}`} onClick={() => setTokenId(id)}><span className={`creature-thumb ${rarity.className}`}><img src={rarity.image} alt="" /></span><span><strong>{rarity.creature}</strong><small>{cards[id]} owned</small></span><i>{tokenId === id ? "✓" : ""}</i></button>)}</div><div className="price-field"><label htmlFor="listing-price">Set your price</label><div><input id="listing-price" value={price} onChange={(e) => setPrice(e.target.value)} inputMode="decimal" /><span>ETH</span></div></div><div className="market-summary"><span>You are listing</span><strong>1× {rarities[tokenId].creature}</strong></div><button className="primary feature-primary" disabled={busy || !account || Number(cards[tokenId]) < 1} onClick={onList}>Publish listing <span>→</span></button></div></details>
      </aside>
      <div className="market-gallery">{visibleListings.length === 0 ? <div className="market-empty"><span>◇</span><h4>No items found</h4><p>Try another rarity or list the first Brickling.</p></div> : visibleListings.map((listing) => {
