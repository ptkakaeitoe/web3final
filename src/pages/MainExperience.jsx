...
export default function MainExperience({
  visible, account, busy, balance, amount, setAmount, mode, setMode, connectionMode,
  liveQuote, previewQuote, trade, boxQuantity, setBoxQuantity, mystBundleEthValue,
  ethBundleCost, mystDiscount, buyBox, connect, ownedPacks, sealedPacks, revealBox,
  collectionEntries, renderCreatureCard, cards, rarities,
}) {
  ...
  return <>
      ...
      <section className={`shop-section app-page ${isMainPage ? "page-active" : ""}`} id="shop">
      ...
      <section className={`inventory-section app-page ${isMainPage ? "page-active" : ""}`} id="my-packs">
        <div className="section-heading">
      ...
      <section className={`collection app-page ${isMainPage ? "page-active" : ""}`} id="collection">
        <div className="section-heading">
        ...
        {collectionEntries.some((entry) => entry.owned) && (
          <div className="collection-group owned-group">
            <div className="collection-group-heading"><div><span>✓</span><strong>Your Bricklings</strong></div><div className="collection-totals"><b>{cards.reduce((total, count) => total + Number(count), 0)} TOTAL OWNED</b><small>{collectionEntries.filter((entry) => entry.owned).length} OF {rarities.length} TYPES DISCOVERED</small></div></div>
